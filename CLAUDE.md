...
- **`romaji.lua` は純粋関数のみ**。FFI も Vim API も触らない。テストは入出力比較で完結する。`to_kana(s, custom_table?)` の第2引数で配列を完全置換できる（撥音/促音/英字ランの look-ahead はテーブル非依存）。
...
- ローマ字テーブルや拗音/促音/撥音のロジックを変えるときは `tests/vime/romaji_spec.lua` のケースを更新する。カスタムテーブル機能を触るときは `tests/vime/{romaji,session,config}_spec.lua` のカスタム関連ケースもセットで確認する。
